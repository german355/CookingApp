...
import com.example.cooking.ui.fragments.profile.AuthFragment;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_ADD_RECIPE && resultCode == RESULT_OK) {
            if (data != null && data.hasExtra("recipeAdded") && data.getBooleanExtra("recipeAdded", false)) {
                // Логика обновления теперь в HomeViewModel, который сам обновит UI
                Toast.makeText(this, R.string.recipe_added_successfully, Toast.LENGTH_SHORT).show();
            }
        } else if (requestCode == 200) {
            if (resultCode == RESULT_OK) {
                // Логика восстановления поиска теперь не нужна, т.к. состояние хранится в ViewModel
            }
    ...
    private void performSearch(String query) {
        ...
        // Найдем HomeFragment и вызовем поиск напрямую
        NavHostFragment navHostFragment = (NavHostFragment) getSupportFragmentManager().findFragmentById(R.id.nav_host_fragment);
        if (navHostFragment != null) {
            Fragment currentFragment = navHostFragment.getChildFragmentManager().getPrimaryNavigationFragment();
            if (currentFragment instanceof com.example.cooking.ui.fragments.HomeFragment) {
                ((com.example.cooking.ui.fragments.HomeFragment) currentFragment).performSearch(query);
            }
        }
    }
    
    /**
     * Сбрасывает результаты поиска и возвращается к отображению всех рецептов
     */
    private void clearSearchResults() {
        Log.d(TAG, "Сбрасываю результаты поиска");
        
        // Найдем HomeFragment и сбросим поиск
        NavHostFragment navHostFragment = (NavHostFragment) getSupportFragmentManager().findFragmentById(R.id.nav_host_fragment);
        if (navHostFragment != null) {
            Fragment currentFragment = navHostFragment.getChildFragmentManager().getPrimaryNavigationFragment();
            if (currentFragment instanceof com.example.cooking.ui.fragments.HomeFragment) {
                ((com.example.cooking.ui.fragments.HomeFragment) currentFragment).clearSearch();
            }
        }
    }
